      ...
      perSystem =
        { config, pkgs, ... }:
        {
          spindrift = {
            ...
            #     podman = "";
            #   };
            #   # cargo registry name (the NAME in a [registries.NAME] table, e.g. in ~/.cargo/credentials.toml) whose token the Registry proxy extracts as the credential; only meaningful when REGISTRY_PROXY_CREDENTIAL_FILE_FORMAT=cargo-credentials, ignored for other formats
