      ...
      perSystem =
        { config, pkgs, ... }:
        {
          spindrift = {
            ...
            #     # value for the orchestrator's code-owned review pass's own --effort flag (issue #2387); pass-through only, no normalization, same accepted values as EFFORT for the active Driver. Overrides the roster reviewer entry's own effort (rosterDefaults.reviewer.effort by default) the same way REVIEW_MODEL overrides the reviewer's model, regardless of whether the roster is the built-in default or a Consumer-supplied explicit one (lib/mkHarness.nix applies the override post-normalize, issue #2512) -- empty means follow the roster, a non-empty value overrides it. Meaningful only under ORCHESTRATOR: the resolved value reaches the orchestrator via the prompt-assembly Handoff's ReviewEffort field (issue #2512), mirroring how ReviewModel reaches it. Like REVIEW_MODEL, this is a nix-build-time-only knob: setting it at dispatch time (REVIEW_EFFORT=... / --review-effort ...) has no runtime effect on an already-built image -- only this flake option, baked in and rebuilt, changes what the review pass runs at (issue #2512; see MIGRATING.md).
            #     reviewEffort = "";
