...
fixed in commit <short-sha-or-description>` for one you actually fixed, or
`<finding> -> won't-fix: <reason>` for one you deliberately left alone (an
escalated finding, per the non-blocking triage above, is a won't-fix here).
No narrative, no headers, no reasoning beyond that one-line reason, and no
pasted diff hunks, file contents, or transcript excerpts — reference the
commit instead of quoting it. A later review round reads this file verbatim
and treats every line as an unverified claim to check against the diff, not
settled fact, so write it precisely enough to survive that check.
