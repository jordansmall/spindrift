...
(empty means the worker made no changes — nothing to integrate). Every
worker's worktree starts from `origin/${BASE_BRANCH}`, not your own tree, so
a worker never sees an earlier slice's work — scope slices to distinct files
where you can. Bring a named branch's diff into your own tree with
`git cherry-pick --no-commit origin/${BASE_BRANCH}..<branch>`, resolving any
conflict by hand, then author the commit yourself so it lands in proper
Conventional Commits form — do this before running that slice's checks or
