...
tool-result cap truncates it, so you end up reasoning about a diff whose end
you cannot see. Write the diff to a file, read `--stat` first for shape, then
read targeted hunks or grep that file for what you need.
...
while it is still pending: the turn ends before the gate finishes, so no
...
${OPEN_PR_PUSH_READ_WRITE_STEP}${OPEN_PR_PUSH_READ_ONLY_STEP}${OPEN_PR_CREATE_READ_WRITE_STEP}${OPEN_PR_CREATE_READ_WRITE_FORGEJO_STEP}${OPEN_PR_CREATE_READ_ONLY_STEP}${OPEN_PR_CREATE_READ_ONLY_SOCKET_STEP}${PR_BODY_CLOSES_STEP}${PR_BODY_LOCAL_REF_STEP}${PR_BODY_LOCAL_NOREF_STEP}The PR opens as a **draft** and stays a **draft** — the launcher flips it to
...
${IF_BLOCKED_PUSH_READ_WRITE_STEP}${IF_BLOCKED_PUSH_READ_ONLY_STEP}${IF_BLOCKED_PR_READ_WRITE_STEP}${IF_BLOCKED_PR_READ_ONLY_STEP}${IF_BLOCKED_PR_READ_ONLY_SOCKET_STEP}3. Leave the issue in-progress — do NOT close it.
