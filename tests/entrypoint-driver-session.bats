...
@test "cold run pins a deterministic session id via --session-id" {
  run bash "$ENTRYPOINT"
  [ "$status" -eq 0 ]
  grep -qE -- '--session-id [0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' "$DRIVER_LOG"
  ! grep -q -- '--resume' "$DRIVER_LOG"
}
...
@test "fix pass resumes the exact session id the initial run pinned" {
  ...
  [ "$status" -eq 0 ]
  local pinned_id
  pinned_id="$(grep -oE -- '--session-id [0-9a-f-]+' "$DRIVER_LOG")"
...
@test "session id is stable across independent cold runs of the same issue" {
  ...
  [ "$status" -eq 0 ]
  local second_id
  second_id="$(grep -oE -- '--session-id [0-9a-f-]+' "$DRIVER_LOG")"
