...
@test "entrypoint passes --agents with only scout when the template carries scout alone" {
  ...
  [ "$status" -eq 0 ]
  [ -s "$DRIVER_AGENTS_FILE" ]
  jq -e 'has("scout") and (has("reviewer") | not)' "$DRIVER_AGENTS_FILE" >/dev/null
...
@test "entrypoint passes --agents with only filer when the template carries filer alone" {
  ...
  [ "$status" -eq 0 ]
  [ -s "$DRIVER_AGENTS_FILE" ]
  jq -e 'has("filer") and (has("scout") | not) and (has("reviewer") | not)' "$DRIVER_AGENTS_FILE" >/dev/null
...
@test "entrypoint passes --agents with only worker when the template carries worker alone" {
  ...
  [ "$status" -eq 0 ]
  [ -s "$DRIVER_AGENTS_FILE" ]
  jq -e 'has("worker") and (has("scout") | not) and (has("reviewer") | not) and (has("filer") | not)' "$DRIVER_AGENTS_FILE" >/dev/null
  jq -e '.worker.prompt | length > 0' "$DRIVER_AGENTS_FILE" >/dev/null
}
...
@test "entrypoint injects a custom agent's prompt generically via AGENTS_PROMPT_FILES" {
  ...
  [ "$status" -eq 0 ]
  [ -s "$DRIVER_AGENTS_FILE" ]
  jq -e 'has("scout") and has("auditor")' "$DRIVER_AGENTS_FILE" >/dev/null
  jq -e '.scout.prompt | length > 0' "$DRIVER_AGENTS_FILE" >/dev/null
  jq -e '.auditor.prompt | length > 0' "$DRIVER_AGENTS_FILE" >/dev/null
}
