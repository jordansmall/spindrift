...
# CODE_FORGE=forgejo: the Box clones from and pushes to a Forgejo/Gitea instance
# over a FORGEJO_TOKEN-authenticated URL (ADR 0038), never github.com. The
# insteadOf rewrite prefix carries the token, so the offline clone only resolves
# if the entrypoint actually embedded FORGEJO_TOKEN as the remote's userinfo.
@test "CODE_FORGE=forgejo clones from a FORGEJO_TOKEN-authenticated Forgejo remote" {
  ...
  # file:// redirect. `git config --get` returns what clone actually wrote --
  # the token-authenticated https URL the entrypoint built and pushes back to.
  run git -C "$WORK_DIR" config --get remote.origin.url
  [ "$status" -eq 0 ]
  [ "$output" = "https://fjtok@forge.test/owner/repo.git" ]
}

# fj credential wiring (issue #1963): clone_repo configures fj
# non-interactively via `fj auth add-key`, piping the token on stdin -- never
# argv, so it never shows up in `ps`. A fake fj on PATH records its args and
# stdin so the test can assert both without a real forgejo-cli.
...
# same bare host clone_repo derives. Without the strip in configure_forgejo_cli,
# fj would store the token under "https://forge.test/" while the git remote
# resolves to "forge.test", silently breaking `fj issue view`/auth lookup.
...
# CODE_FORGE=local: under rootless podman the Box's mapped uid never matches the
# host-owned mount's uid, so git's dubious-ownership guard rejects the mount
# before a single object is copied (#1720). GIT_TEST_ASSUME_DIFFERENT_OWNER=1 is
# git's own test-suite knob for faking that mismatch, which an unprivileged bats
# sandbox can't produce for real.
...
# CODE_FORGE=local: a ref left at origin/agent/issue-7 by an earlier, abandoned
# attempt must trigger neither the adoption path's `gh pr list` -- a forge
# network call CODE_FORGE=local must never make (ADR 0033) -- nor a push back to
# the read-only mount. The Box starts BRANCH fresh from base every time.
...
@test "CODE_FORGE_REMOTE_URL is ignored when CODE_FORGE is unset (github default)" {
  ...
  # CODE_FORGE at its schema default, the same value an unset var would carry.
  local other_remote="$BATS_TEST_TMPDIR/other-remote.git"
  git init --bare -q "$other_remote"
