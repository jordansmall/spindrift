...
# resultTextExtractFnBody) against its own canonical fixture transcript -- the
# cross-half anchor, since the Go half's tests consume the identical fixture
# files. Driven entirely by DRIVER_OUTCOME_MANIFEST, so a new Driver registry
# entry with its own testdata/outcome-fixture.jsonl is covered with no edits
# here.
...
@test "every registered driver's _driver_extract_outcome/_driver_extract_near_miss_outcome/_driver_extract_result_text parse its own canonical fixture" {
  ...
  for driver in $(jq -r 'keys[]' "$DRIVER_OUTCOME_MANIFEST"); do
    ...
    # _driver_extract_result_text does no grep/classification/tail -1 filtering
    # itself (the Go side's outcome.LastFieldedOutcomeLine does that), and its
    # raw multi-line output differs per driver, so only its last leading-token
    # line is checked against the canonical outcome line.
...
@test "every registered driver's _driver_extract_outcome/_driver_extract_near_miss_outcome behave identically across valid, markdown-wrapped, colon-delimited, and near-miss lines" {
  ...
  # stream envelope (claude: a "result" event; opencode: a "text" part event) --
  # per-Driver wire-format knowledge the unified extractor never sees, so
  # branching on $driver here is test-fixture plumbing, not the copy-paste the
  # Nix refactor eliminated.
  _build_event() {
    local d="$1" t="$2"
    case "$d" in
...
@test "every registered driver's _driver_extract_result_text tolerates a malformed non-JSON tail line" {
  : "${DRIVER_OUTCOME_MANIFEST:?DRIVER_OUTCOME_MANIFEST must be set -- run via nix (checks-inbox), not a bare bats invocation}"
  # Regression guard: resultTextExtractFnBody's pipeline must end in `|| true`
  # like its siblings, so a jq parse failure on a malformed line in the driver's
  # raw stream log doesn't propagate through the pipe under `set -eo pipefail`
  # and abort the caller before the SPINDRIFT_OUTCOME line is ever handled.
