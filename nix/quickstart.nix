...
#
# vendorHash is lib/build-constants.nix's launcherVendorHash — the same root
# lib/mkHarness.nix's launcherBin and nix/checks/go.nix's launcherGoModules
# import (same cmd/launcher go.mod/go.sum, same full source tree — vendoring
# is a whole-module computation independent of subPackages), so all three
# can't drift apart.
{ pkgs }:
let
