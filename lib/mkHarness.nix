...
let
  ...
  # Generated Go source for cmd/launcher/flagtable_gen.go.
  # Non-secret schema knobs each become a flagEntry row; secret knobs are
  # excluded so they never appear in `ps` output or shell history.
  # Rows are emitted in lexicographic attrset order (lib.mapAttrsToList).
  renderFlagTableGo =
    let
      nonSecretSchema = lib.filterAttrs (_: e: !(e.secret or false)) schema;
      toKebab = env: lib.toLower (lib.replaceStrings [ "_" ] [ "-" ] env);
      flagKind = e: if builtins.isInt (e.default or null) then "int" else "string";
      rows = lib.concatStrings (
        lib.mapAttrsToList (_: e:
          "\t{env: \"${e.env}\", flag: \"${toKebab e.env}\", kind: \"${flagKind e}\"},\n"
        ) nonSecretSchema
      );
    in
    # The header comment must match the committed file exactly for the
    # launcher-flag-table check to pass (diff is byte-for-byte).
