...
rec {
  ...
  # cmd/launcher/internal/daemon/shippeddefaults_gen_test.go content: the Go
  # form of lib/env-schema.nix's five daemon tuning knobs, keyed by env var
  # name, so backoff_test.go and breaker_test.go assert against it instead of
  # restating the shipped defaults by hand (issue #3619).
  renderDaemonKnobDefaultsGo =
    schema:
    let
      # One list, not five hand-written map rows, so a sixth knob only needs
      # a name added here rather than a new render call.
      knobAttrs = [
        "daemonIdleFloor"
        "daemonIdleCap"
      ...
      # daemonBreakerThreshold.default is a Nix int; every other knob's
      # default is already the Go string literal it needs to become.
      renderDefault =
        attr:
        let
          d = schema.${attr}.default;
        in
        if builtins.isInt d then toString d else d;
      rows = concatStrings (
        map (attr: "\t\"${schema.${attr}.env}\": \"${renderDefault attr}\",\n") knobAttrs
      );
    in
