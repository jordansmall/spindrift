...
rec {
  ...
  # renamed, name), plus the single `TriageLabelMeta` map every tier feeds
  # into except `recoverable` (never a real created label -- see
  # lib/labels.nix's doc comment) and `triggerOnly` (workflow-only
  # vocabulary, never colored/created by doctor). Emits unaligned Go, like
  ...
  # path string), each key rendered as a PascalCase `Path<Key>` Go
  # identifier.
  renderQuickstartPathsGo =
    ...
    let
      capitalize =
        key:
        toUpper (builtins.substring 0 1 key) + builtins.substring 1 (builtins.stringLength key - 1) key;
      # builtins.toJSON, not a hand-wrapped "${value}", so a path round-trips
      # into a valid Go string literal -- the same escaping treatment
      # renderAgentPathsGo's renderConst uses for Go string literals.
      renderConst =
        key: value:
        "// Path${capitalize key} is the nix option path for the quickstart wizard's ${key} knob.\n"
        + "const Path${capitalize key} = ${builtins.toJSON value}\n";
  ...
  renderAgentPathsGo =
    ...
    let
      ...
      capitalizeWord =
        w:
        toUpper (builtins.substring 0 1 w) + toLower (builtins.substring 1 (builtins.stringLength w - 1) w);
