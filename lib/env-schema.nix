...
in
{
  ...
  registryProxyRoutesFile = {
    ...
    group = "infra";
    doc = "path to a TOML routes file declaring registry routes (ADR 0045); each route binds match-host, optional upstream-base-url (base path permitted; declaring it selects the legacy base-path-join serving model) -- omitting it selects host-rooted serving instead (ADR 0047): the route serves the whole matched host under its prefix, forwarding the verbatim remaining path onto the upstream origin with no base-path join, and its requests are checked unconditionally (regardless of enforce-allowlist) against a path-set the launcher derives host-side from the Target repo's own committed registry config at dispatch time -- a request outside that derived set is answered 403 naming the policy and the derived set, before any upstream dial and with no credential attached; a host-rooted route therefore needs a snapshot of the Target repo's committed config to derive from, and the launch fails naming the route when none is available -- under CODE_FORGE=local that snapshot is the bare Accumulation repo's own BASE_BRANCH ref (ADR 0033), read host-side before any Box starts -- never a checkout around the launcher's cwd, so a dirty or unpushed working tree can neither widen nor narrow the derived set; under every other Code Forge it is a Target-repo checkout resolved from the launcher's cwd and positively identified against the forge's own remote (ADR 0047 #3310), optional auth-scheme (bearer default; basic and header:<Name>), optional enforce-allowlist (default false, advisory: a route-relative path outside the derived allowlist is logged and relayed, never refused, since derived patterns aren't provably complete and a false denial looks like a registry outage to the Agent; when true, checked after the TCP shared-secret and GET/HEAD gates and before credential attachment, so a non-matching path is answered 403 naming the policy and pattern set and never dials upstream or gets a credential attached; per-route only -- tightens, never loosens, the read-only posture; on a cargo route, note the Forwarder-rewritten dl endpoint is itself outside the derived allowlist, so enabling it there also 403s crate downloads), optional cargo-registries (names of the Target repo's [registries.NAME] entries this route serves, each restricted to letters, digits, '-', and '_'; cargo binds to a named registry through source replacement rendered into the Box's $CARGO_HOME/config.toml, keyed off the repo's own un-rewritten .cargo/config.toml -- not a rewrite of that file -- so when cargo-registries is present it restricts which of the repo's declared registries get a replacement stanza on this route, and when absent every declared registry whose index host matches this route does; the CARGO_REGISTRIES_<PROXY-SOURCE-NAME>_TOKEN placeholder this produces is keyed to cargo's own replacement source name, not the repo's registry name, since cargo binds credential lookups to the replacement source once a source is replaced -- ADR 0044 #3201 amendment; when the repo's own .cargo/config.toml already claims a declared registry's index URL under its own [source.NAME], the render reuses that name instead of minting spindrift-upstream-<name> so the stanzas merge rather than colliding, and on a repo that also replaces crates-io with that source, a plain crates-io fetch chains crates-io -> that named source -> this route, so the crates-io route's own prefix may see no traffic and this route's credential and allowlist policy governs those fetches -- ADR 0044 #3248 amendment), optional allow (empty in the happy path; a list of extra path patterns that extend a host-rooted route's derived enforced path-set for a path shape the Target repo's own committed config doesn't expose -- e.g. an Artifactory-style sibling download endpoint -- ADR 0047 #3258; each pattern is validated at parse time and must already be in canonical absolute-path form -- a leading slash, no trailing slash, and no '.' or '..' segment -- rejected otherwise with an error naming the route and the offending pattern; the bare root pattern '/' is rejected too, since it would blanket-authorize the whole host, exactly the off switch this key must never be; allow is only valid on a host-rooted route (one that omits upstream-base-url) -- declaring it on a legacy route that also sets upstream-base-url is itself a parse-time error naming the route, since that serving model is enforced by a different, hardcoded mechanism allow never reaches; a request matching an allow pattern forwards with the route's credential exactly like a request matching a derived path, the same 403-or-forward check and code path; allow only ever widens the enforced set, never a way to disable or narrow enforcement -- there is no enforce = false and this key doesn't add one), optional gradle-path (only meaningful on a host-rooted route -- declaring it alongside upstream-base-url is itself a parse-time error naming the route, the same as allow on a legacy route -- since gradle has no committed in-tree config file spindrift can scan to derive a path from the way it does for npm/yarn/pnpm/cargo, so an operator declares gradle's own path on the host directly here; must be an absolute path already in canonical form -- a leading slash, no trailing slash, no '.', '..', or empty segment -- and must not be the bare root '/' or contain a dollar sign, a backtick, or a backslash, rejected otherwise with an error naming the route; omitting it on a host-rooted route leaves gradle's binding inert -- no repository redirection at all -- falling back to whatever repositories the build declares itself), optional go-path (only meaningful on a host-rooted route -- declaring it alongside upstream-base-url is itself a parse-time error naming the route, the same as gradle-path and allow; go has no committed in-tree config file spindrift can scan to derive a path from the way it does for npm/yarn/pnpm/cargo -- a go.mod names no registry host -- so an operator declares go's own path on the host directly here; must be an absolute path already in canonical form -- a leading slash, no trailing slash, no '.', '..', or empty segment -- and must not be the bare root '/' or contain a dollar sign, a backtick, or a backslash, rejected otherwise with an error naming the route; the declared path joins the route's enforced path-set as operator-owned policy, exactly as gradle-path's does, so go module fetches under it forward with the route's credential; when present, GOPROXY is bound to the Forwarder URL carrying that full path; omitting it on a host-rooted route leaves go unbound through that route entirely -- no GOPROXY export at all, so go falls back to its own default public proxy rather than to a bare-root URL that was never declared for it), and an optional credential source reference (exactly one when present; an absent credential key leaves that route unauthenticated, a plain pass-through); the file carries credential source REFERENCES (env var names, file paths), never secret values; the inbound Authorization header is stripped from every proxied request, on both host-rooted and base-path routes, before any route credential is attached (ADR 0047); unset disables the registry proxy entirely; this is the only declaration surface for registry routes; the proxy routes strictly by a per-route path prefix slugged from each route's match-host -- a request under no known prefix is refused before any upstream is dialed";
    flakeOption = true;
    legacySettingsExempt = true;
    boxEnv = false;
  };
