...
- **podman** (or set `infra.runtime = "docker"`; `infra.runtime = "rancher"` for Rancher
  Desktop in containerd mode, driven via `nerdctl`; or `infra.runtime = "bwrap"` for
...
this template's `flake.nix` doesn't reference `./skills`), a
...
| [`docs/flake-options.md`](docs/flake-options.md) | Schema-generated reference for every schema-generated flake option, grouped by domain (ADR 0037) — attr path, env var, default, description; structural options (`roster`, `skills`, `driver`, ...) aren't schema-generated and are documented in the Option surface chapter of `docs/reference.md` instead; regenerated by `nix flake check` |
