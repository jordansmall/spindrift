...
| `perSystem.spindrift.agents.models.workerTimeout` | `WORKER_TIMEOUT` | — | timeout applied to each parallel worker subagent's run (issue #2059); pass-through only, no normalization. Meaningful only under ORCHESTRATOR: entrypoint.sh threads this value to the orchestrator's --worker-timeout flag. |
| `perSystem.spindrift.agents.models.workerWorkDir` | `WORKER_WORK_DIR` | — | worktree directory the orchestrator hands each parallel worker subagent (issue #2059); pass-through only, no normalization. Meaningful only under ORCHESTRATOR: entrypoint.sh threads this value to the orchestrator's --worker-work-dir flag. |
