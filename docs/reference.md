...
`forgejo` when the repo's remote is on codeberg.org. For any other
non-GitHub host, it asks which one to use. `local` and `jira` are
experimental, and you select them by editing `issues.tracker` in the
generated `flake.nix`.
