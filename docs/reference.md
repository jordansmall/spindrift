...
live) are independent axes. `ISSUE_TRACKER` selects the tracker and
`CODE_FORGE` the forge, so pairings like Jira issues with GitHub PRs work.

The [Quickstart wizard](../README.md#quick-start) provisions `github`, or
`forgejo` when the repo's remote is on codeberg.org (it asks which on any
other non-GitHub host). `local` and `jira` are experimental and reachable only
by hand-editing `issues.tracker` in the generated `flake.nix`.
