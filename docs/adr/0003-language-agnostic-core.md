...
illustration — a single `infra.image.packages = p: [ p.go ]` straight from
nixpkgs, no overlay or extra input — precisely to show the core carries no
language machinery. A stack that needs one (e.g. `rust-overlay` for pinned
Rust channels) adds the overlay and input in the *consumer's* flake, never
the core.
