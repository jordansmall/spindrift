...
// repoRequirementExemption reports whether c is exempt from the
...
// reached from inside the Box.
func repoRequirementExemption(c config) bool {
	return repoRequirementExemptionFor(resolveCapabilitySignals(c.codeForge, c.issueTracker), c)
}

// repoRequirementExemptionFor is repoRequirementExemption's logic over an
// already-resolved sig, so a caller building multiple rows for the same c
// (launcherRequiredKnobChecks' repo-slug and gh-token rows) resolves
// capability signals once instead of once per Probe.
	...
	return []doctor.Check{
		{
			...
			Remedy: "set REPO_SLUG=owner/repo (the target GitHub repository)",
			Probe: func() error {
				if !repoRequirementExemptionFor(sig, c) && c.repoSlug == "" {
		...
		},
		{
			Name:   "git-user-email",
			Tier:   doctor.Required,
			Remedy: "set GIT_USER_EMAIL, or configure git user.email on the host",
			Probe: func() error {
				if c.gitUserEmail == "" {
		...
		},
		{
			Name:   "gh-token",
			Tier:   doctor.Required,
			Remedy: "set GH_TOKEN (fine-grained PAT scoped to the single target repo: Issues RW, Contents RW, Pull requests RW, Metadata R)",
			Probe: func() error {
				if !repoRequirementExemptionFor(sig, c) && c.ghToken == "" {
