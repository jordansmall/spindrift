...
func TestDoctorCheckSets_PutsPodmanMachineMemoryInReportNotClassify(t *testing.T) {
	c := minimalValidConfig()
	c.runtime = "podman"
	...
	for _, ch := range classify {
		if ch.Name == podmanMachineMemoryCheckName {
			t.Errorf("classify contains podman-machine-memory, want it excluded (environment concern, not a configuration fault)")
		}
	}
