...
// This test pins the handoff schema (issue #1997): a fresh implementor pass
// continues without a transcript, so every documented field has to survive
...
// ReviewFindings (issue #2037) carries the review pass's own
// Blocking/Non-blocking text, distinct from the bare LastVerdict word.
...
// PassSummaryPath (issue #2549) points at the most recent implement/fix
// pass's free-form summary of what it did and what remains.
...
// DispositionsLogPath (issue #2550) points at the per-run, append-only
// dispositions log seedReviewPromptFromState reads.
...
// ReviewedCommitAnchor (issue #2551) holds the git commit SHA the
// orchestrator's repo workdir was at when the most recent review pass ran.
...
// DecisionsPath (issue #2695) points at the implement/fix pass's
// per-decision file.
...
// DecisionsLogPath (issue #2695) points at the per-run, append-only
// decisions log seedPromptFromState reads.
...
// This test pins the pass-one production path (issue #1997): --state-file
// defaults to a fixed tmp path nothing has written yet, so a missing file
// has to read as "no handoff yet". An error there would fail the
// orchestrator's first invocation on any box before it reached driver-exec.
...
// An empty path is how a caller disables the artifact entirely, so it reads
...
// WriteRunState must never truncate in place (issue #2008): the new content
// becomes visible at path only through a rename swap, so a reader holding the
// old fd still sees complete old content, and a kill mid-write leaves either
// the old file or an orphaned temp file, never a half-written one at path.
...
// The fixture is hand-written rather than produced by WriteRunState on
// purpose: it stands in for a state file a prior pass wrote before this
// package existed, and covers every field the pre-extraction orchestrator
// wrote, not just the ones exercised elsewhere in this file (AC2).
...
// A file that exists but does not parse (a partial write from a killed prior
// pass, or hand-edited garbage) must error rather than silently discard the
// handoff data it was supposed to carry.
...
// IsEmpty (issue #2552) is true only for the zero value. DoneSlices and
// RemainingSlices stay out of the non-empty cases on purpose: they are
// dispatch-internal bookkeeping (issue #2059's dedup in
// orchestrator/dispatch.go) that seedPromptFromState never renders, so alone
// they must not seed a "Run-state handoff" block with nothing in it (#2549).
func TestRunStateIsEmpty(t *testing.T) {
	...
	// DispositionsPath joins DoneSlices/RemainingSlices in the "set but still
	// IsEmpty" case (issue #2550 review finding): seedPromptFromState, the
	// only caller, never renders it. seedReviewPromptFromState's own narrower
	// check governs the round-N review prompt DispositionsPath seeds.
	...
	// ReviewedCommitAnchor is the same case: only seedReviewPromptFromState
	// (issue #2551) reads it.
	...
	// DecisionsPath and DecisionsLogPath join them in the same case (issue
	// #2695): seedPromptFromState re-reads DecisionsLogPath's file itself
