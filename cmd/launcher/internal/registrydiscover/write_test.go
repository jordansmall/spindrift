...
// registryroutes.Parse into the same match-host, upstream-base-url,
...
func TestRender_NetrcRouteRoundTripsThroughParse(t *testing.T) {
	...
	if got.UpstreamBaseURL != "https://crates.acme.example/api/v1" {
		t.Errorf("UpstreamBaseURL = %q, want https://crates.acme.example/api/v1", got.UpstreamBaseURL)
	}
