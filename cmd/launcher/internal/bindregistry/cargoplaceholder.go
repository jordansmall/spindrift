...
// mentions localURL, i.e. a registry whose index was actually rewritten to
// point at the Forwarder. [source.NAME] tables are deliberately excluded:
// only [registries.*] is what cargo's credential-provider/token mechanism
// applies to.
//
...
func ParseCargoRegistryNames(content, localURL string) []string {
	...
	for _, line := range lines {
		...
		if inRegistriesSection && !sectionRewritten {
			...
			if ok && strings.TrimSpace(key) == "index" && strings.Contains(trimmed, localURL) {
