...
// Issue #1939: settle.postBlockedNoteComment posts Note as a public comment on
// status=blocked, and a Dispatch reuses one nonce across every retry, so a nonce
// leaked into Note lets a comment author replay it against a later attempt.
...
// The heuristic is deliberately narrow: a trailing "nonce=<value>" counts as
// the field only when <value> has no spaces, the shape a genuine single-token
// nonce always has. Note text containing "nonce=" plus more words is left alone.
...
// This pins ReadyBeforeNote to the substring test the deleted bash note-field
// split used (agent/entrypoint.sh, commit a43506a8): bounded at the first
// " note=", literal "status=ready" token match. It is deliberately looser than
// Parse's grammar (an empty landing still counts as ready) and narrower on the
// note field (a "status=ready" mention inside note text must not count).
...
// Issue #2223: the synthetic=true field must not bleed into Note, which follows
// it in the grammar.
...
// Issue #2223: a non-synthetic Outcome's rendered line must stay byte-identical
// to the pre-Synthetic-field wire format.
...
// The scanner-mechanics tests for lastInLog and lastSelfReportInLog live in
// outcome_internal_test.go (package outcome), since those scanners are
// unexported (issue #2260). The Resolve tests below cover the policy-level
// behavior: which tier wins, and Resolve's own Skipped field.
...
// encodePRIntent encodes title and body the way a read-only Box's prompt
// fragment instructs: title, a blank line, then body.
...
// CreateDraftPR passes title and body as argv, never through a shell, so this
// grammar must hand shell and markdown metacharacters through byte-for-byte
// rather than escaping or stripping anything.
...
// A verified PR-intent line must not be suppressed by a later line that carries
// the token without verifying: an untrusted author's echoed mention, or a
// stale-nonce line from another run. LastCommentLineInLog gives the same
// guarantee. LastInLog does not, because SPINDRIFT_OUTCOME is not nonce-gated.
...
// The same non-shadowing guarantee in the opposite order: the scan must not
// short-circuit on an earlier rejected line and miss a later verified one.
...
// A line whose nonce does not match this run's own is the shape an untrusted
// author's echoed text takes, since they wrote it before the nonce was minted.
// It must never open a PR, and must surface as an error the caller can warn on
...
// An empty expectedNonce, the zero value a caller might pass by mistake, must
// never verify a line, even one carrying no nonce field at all. The fixture has
// only one field after the token, so under issue #2089 it also reads as a doc
// example rather than a signal attempt: it stays silently non-matched
...
// The decoder is strict: it rejects a malformed payload outright rather than
...
// Issue #2089: a line leading with the token but carrying only one field after
// it (a doc example missing the base64 payload) is not a signal attempt, so it
// neither verifies nor warns.
...
// The line must be found inside a real stream-json JSONL log shape, where a
// multi-line block collapses onto one JSON-escaped physical line and never
// matches an exact-line marker scan. That dogfood failure (issue #1921) is why
// this signal moved to a single line at all.
...
// When the Box's narration shares the assistant-message text field with the
// marker, stream-json encodes the separating newline as a literal backslash-n,
// landing the 'n' directly against the token's first letter with no real
// whitespace. Same collapse class as issue #1921, but for a token that does not
// start its JSON text field, which SurvivesStreamJSONCollapse does not exercise.
...
// The LastFieldedOutcomeLine and LastNearMissOutcomeLine tests pin the deleted
// bash extractor's filter-then-last semantics (outcomeExtractFnBody/
// outcomeExtractNearMissFnBody, git show a2addd2b:lib/drivers/claude.nix): a
// token-leading line is "fielded" purely by carrying a landing= and a status=
// marker, any value included, never by Parse's grammar, and the last one wins.
...
// Bug 1: the deleted bash's filter only checked for the literal "landing="
// marker, so an empty landing value still counts as fielded, unlike
// outcome.Parse, which rejects an empty landing as ErrNearMiss.
...
// Bug 2: the deleted bash filtered to fielded lines first, then took the last of
// those, so a later token-leading line carrying neither field marker (a bare
// paraphrase) must never shadow an earlier genuine fielded line.
...
// The near-miss complement: when no fielded line exists at all, the last
// token-leading line that fails the fielded test is the near-miss candidate.
...
// The nonce-gated COMMENT signal prefers the last line that actually verifies,
// unlike LastInLog's unconditional last-line-wins, so an attacker cannot
// suppress a real verdict or blocked-note by posting COMMENT-shaped noise after
// it.
...
// A COMMENT line whose nonce does not match must be ignored and reported via a
// non-nil error rather than ever being posted: an untrusted author echoing the
// token, or a stale nonce from another run, never verifies.
...
// Every token-bearing line that fails to verify is counted rather than collapsed
// into a bool, so a caller learns how many spoof attempts it saw.
...
// The decoder is strict: it rejects a payload that fails to decode even when the
// nonce is correct, rather than best-effort decoding it.
...
// Issue #1940, issue #1921's twin: a stream-json JSONL box log collapses the
// printed line onto one physical line, JSON-escaping the trailing newline into a
// literal backslash-n abutting the base64 payload. The single-line token must
// still decode cleanly out of that shape, unlike the exact-line
...
// Issue #2018's 1-to-many contract: several distinct verifying lines in one run's
// log each contribute their payload, in encounter order. Unlike the singleton
// last-wins scanners, issue filing is not a single overwritable slot.
...
// A mismatched-nonce line (issue #1939's shape: an echoed token written before
// this run's nonce was minted) is dropped from the payloads but still counted
// via the rejected-count return, as the singleton scanners count a non-verifying
// token-bearing line (issue #2976), so a caller can warn instead of the drop
// staying silent.
...
// Issue #2068: when the Filer runs as a subagent, its single line appears twice
// in the raw stream-json log, once as the subagent's own assistant event and
// once echoed in the parent implementor's tool_result event. Both verify and
// decode identically, so without dedup one finding would file two issues.
...
func TestResolve(t *testing.T) {
	cases := []struct {
		...
		{
			// Before #2274 this nonce-free line was excluded from the genuine tier
			// and surfaced via the self-report fallback.
		...
		{
			// Issue #2973: a tool_result echo embedding the token mid-JSON, with
			// field markers but no leading-token line anywhere in the log, resolves
			// as plain no-outcome. TestLastInLog_ToolResultEchoIsNotFound pins the
		...
		{
			// Issue #2973: a later log holding only a mid-sentence mention, which is
			// field-bearing but not a candidate, must not shadow an earlier log's
			// genuine leading-token outcome. Before the mention-tier fallback was
			// deleted, that near-miss overrode the earlier winner via Resolve's
			// err != nil precedence arm.
...
// Issue #2929: this pins IsGenuineOrSynthetic's verdict across all three
// provenance tiers.
...
// Issue #2268 slice 1: Resolved.SelfReport and Resolved.SelfReportFound carry
// the self-report signal alongside whichever tier won Outcome and Provenance,
// not only when the self-report tier is Resolve's last-resort fallback.
func TestResolve_SelfReportAlwaysPopulated(t *testing.T) {
	...
	t.Run("no self-report line at all leaves SelfReportFound false", func(t *testing.T) {
		// The only leading-token line is the synthetic backstop, which
		// lastSelfReportInLog excludes, so the log holds no self-report line.
		path := writeLog(t,
			"some output",
			"SPINDRIFT_OUTCOME issue=9 landing=agent/issue-9 status=blocked synthetic=true note=driver exited nonce=the-nonce",
		)
...
// Issue #2343 slice 1: an I/O error during the self-report tier's log walk
// surfaces on Resolved.SelfReportError while Found, Provenance and Outcome,
// driven by the genuine/synthetic tier, stay unaffected. The change is purely
// additive, so a caller ignoring SelfReportError sees the old behavior.
...
// Issue #2343 slice 1's silent-skip guarantee: a bad log between two good ones
// is skipped and the self-report walk keeps going. Pass-1 gives an early
// self-report, pass-2 (a directory) errors, and pass-3 gives both the genuine
// outcome and the winning later self-report. SelfReportError still surfaces
// pass-2's error: it is a last-seen tracker, independent of the winning report.
