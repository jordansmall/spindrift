...
func TestDoctorRun_Forgejo_CreatesTriageAndResearchLabels(t *testing.T) {
	...
	err := doctor.Run(fc, cf, cfg, doctor.NewReporter(&buf), bufio.NewScanner(strings.NewReader("y\n")), true, nil)
	if err != nil {
...
func TestDoctorRun_Forgejo_MissingResearchLabelsAdvisoryOnly(t *testing.T) {
	...
	var buf bytes.Buffer
	err := doctor.Run(fc, cf, cfg, doctor.NewReporter(&buf), bufio.NewScanner(strings.NewReader("")), false, nil)
	if err != nil {
