...
func TestValidateJiraEnv_RequiresBaseURLProjectKeyToken(t *testing.T) {
	if err := forge.ValidateJiraEnv("https://example.atlassian.net", "PROJ", "tok", ""); err != nil {
		t.Fatalf("fully configured jira env should validate: %v", err)
	}
	if err := forge.ValidateJiraEnv("", "PROJ", "tok", ""); err == nil || !strings.Contains(err.Error(), "JIRA_BASE_URL") {
		t.Errorf("ValidateJiraEnv should require JIRA_BASE_URL, got: %v", err)
	}
	if err := forge.ValidateJiraEnv("https://example.atlassian.net", "", "tok", ""); err == nil || !strings.Contains(err.Error(), "JIRA_PROJECT_KEY") {
		t.Errorf("ValidateJiraEnv should require JIRA_PROJECT_KEY, got: %v", err)
	}
	if err := forge.ValidateJiraEnv("https://example.atlassian.net", "PROJ", "", ""); err == nil || !strings.Contains(err.Error(), "JIRA_TOKEN") {
