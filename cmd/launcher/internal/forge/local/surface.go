...
// the counterpart to Merge's landing: it makes a completed broad ticket's
// assembled work visible in the operator's checkout without ever switching
// pwd off its current branch or touching origin. Idempotent: fetching an
// already-current branch is a no-op (surfaced=false), and fetching a
// fast-forwardable one advances it (surfaced=true).
//
...
// hand and let a later run pick it back up: parent is currently checked out
// in pwd (mirrors console_freshness.go's checkCheckoutSafe), or the local
