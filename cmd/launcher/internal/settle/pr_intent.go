...
// forge.DraftPRCreator hook) from the Box's PR-intent block. branch is the
...
// way — a missing/malformed bundle, a missing/malformed PR-intent block, or
...
func (s *Settle) hostMediateDraftPR(num, branch string, result dispatch.Result) (string, bool) {
	...
	// that left no usable PR-intent block has nothing worth relaying a
	// branch for.
	title, body, ok := parsePRIntent(result)
	if !ok {
		return s.blockHandoff(num, branch, errors.New("no usable PR-intent block found in the box's log"))
	}
...
// parsePRIntent extracts a title and body from result's PR-intent block: by
// convention the first line is the title and the remainder — after a blank
// line, or immediately when the box omitted one — is the body. Returns
// ok=false when no block was found or the block carried no title, the two
// malformed shapes hostMediateDraftPR must block on rather than pass an
// empty title to CreateDraftPR. An empty body (a title-only block) is a
