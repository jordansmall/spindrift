...
// fragments (rendered into Result.ReviewPromptText, not Result.Prompt --
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !strings.Contains(prompt, "cat /issue-snapshot.md") {
				t.Errorf("ReviewPromptText missing the frozen issue-snapshot read for tracker %s:\n%s", tc.name, prompt)
			}
			for _, oldMechanism := range []string{
				"gh issue view",
				"fj issue view",
				...
				if strings.Contains(prompt, oldMechanism) {
					t.Errorf("ReviewPromptText still contains old live-read mechanism %q for tracker %s, want only the snapshot file read:\n%s", oldMechanism, tc.name, prompt)
				}
			}
		})
	}
...
func TestAssembleOrchestratorReviewerDrop(t *testing.T) {
	...
	// Issue #2547 re-pointed review-issue-read-github.md at the frozen
	// /issue-snapshot.md, so the tracker-read fragment no longer carries an
	// ISSUE_NUMBER to substitute. review-prompt.md's own SPEC dimension does,
	// unconditionally -- the #3222 CODE_REVIEW_UNBAKED caveat is about
	// Result.Prompt's code-review pair, not this review prompt's body.
	if !strings.Contains(result.ReviewPromptText, "issue #2349") {
